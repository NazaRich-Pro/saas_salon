...
from .models import User, DeviceSession, LoginAttempt
from .serializers import (
    LoginSerializer, TwoFAVerifySerializer, TwoFASetupSerializer,
    RefreshTokenSerializer, LogoutSerializer, UserProfileSerializer,
    DeviceSessionSerializer, ChangePasswordSerializer, LoginAttemptSerializer
)
from .jwt_utils import (
    generate_access_token, generate_refresh_token, rotate_refresh_token,
    verify_refresh_token, revoke_all_user_tokens, terminate_device_session,
    parse_device_info, create_or_update_device_session
)
from .totp_utils import (
    setup_2fa_for_user, enable_2fa_for_user, disable_2fa_for_user,
    verify_totp_code, requires_2fa
)
from .throttling import (
    LoginRateThrottle, EmailLoginRateThrottle, TwoFAThrottle,
    check_login_attempts, record_login_attempt, is_captcha_required
)
...
def login(request):
    ...
    refresh_token_str, refresh_token_obj = generate_refresh_token(user, device_id, device_info)
...
def refresh(request):
    ...
    device_info = parse_device_info(request)
    new_refresh_token_str, new_refresh_token_obj = rotate_refresh_token(refresh_token_str, device_info)
