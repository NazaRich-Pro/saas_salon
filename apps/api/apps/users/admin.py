...
from .models import User, RefreshToken, DeviceSession, LoginAttempt


@admin.register(User)
class UserAdmin(BaseUserAdmin):
    list_display = ['email', 'is_superadmin', 'is_active', 'twofa_enabled', 'failed_login_attempts', 'created_at']
    ...
    fieldsets = (
        ...
        ('Permissions', {
            'fields': ('is_active', 'is_staff', 'is_superadmin', 'is_superuser', 'groups', 'user_permissions'),
        }),
        ('2FA', {'fields': ('twofa_enabled', 'twofa_secret')}),
        ('Security', {'fields': ('failed_login_attempts', 'last_failed_login', 'locked_until')}),
...
class RefreshTokenAdmin(admin.ModelAdmin):
    list_display = ['user', 'device_name', 'device_id', 'is_revoked', 'expires_at', 'last_used_at', 'created_at']
...
class DeviceSessionAdmin(admin.ModelAdmin):
    list_display = ['user', 'device_name', 'device_type', 'os', 'browser', 'ip_address', 'is_active', 'last_activity']
...
class LoginAttemptAdmin(admin.ModelAdmin):
    list_display = ['email', 'success', 'failure_reason', 'required_2fa', 'passed_2fa', 'ip_address', 'attempted_at']
