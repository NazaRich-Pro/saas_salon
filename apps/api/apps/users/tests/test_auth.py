...
from apps.users.models import User, RefreshToken, DeviceSession, LoginAttempt
...
class TestLogout:
    ...
    def test_logout_all_devices(self, api_client, test_user):
        ...
        for i in range(3):
            generate_refresh_token(test_user, f'device-{i}', {
                'device_name': f'Device {i}',
                'user_agent': 'Mozilla/5.0',
                'ip_address': '127.0.0.1'
            })
        
        assert RefreshToken.objects.filter(user=test_user, is_revoked=False).count() == 3
        ...
        assert RefreshToken.objects.filter(user=test_user, is_revoked=False).count() == 0
