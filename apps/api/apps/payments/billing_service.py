...
class BillingService:
        ...
        subscription, created = SaaSSubscription.objects.get_or_create(
            ...
            defaults={
                ...
                'monthly_price_kgs': self.calculate_monthly_price(
                    'SOLO' if self.tenant.type == 'SOLO' else 'SALON',
                    self.tenant.seats
                )
            }
        )
    ...
    def get_billing_status(self) -> Dict:
        ...
        booking_blocked = is_suspended or (is_grace and subscription.grace_until and subscription.grace_until < today)
    ...
    def update_seats(self, new_seats: int) -> SaaSSubscription:
        ...
        self.subscription.monthly_price_kgs = self.calculate_monthly_price(plan, new_seats)
        self.subscription.save(update_fields=['seats', 'monthly_price_kgs', 'updated_at'])
    ...
    def can_create_booking(self) -> tuple[bool, Optional[str]]:
        ...
        if status == 'GRACE':
            if self.subscription.grace_until and self.subscription.grace_until >= date.today():
