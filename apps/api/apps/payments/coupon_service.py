...
class CouponService:
    ...
    def validate_and_apply(
        ...
        # Check rules
        if not self._check_rules(coupon, appointment_total, service_ids, appointment_datetime):
    ...
    def _check_rules(
        ...
        if allowed_categories and service_ids:
            ...
            if not any(str(cat_id) in allowed_categories for cat_id in service_categories):
                return False
    ...
    def apply_coupon_to_appointment(
        self,
        appointment,
        code: str
    ) -> Decimal:
        ...
        service_ids = [
            str(aps.service_id) 
            for aps in appointment.services.all()
        ]
        ...
        appointment.internal_notes = f"Купон {code} применен\n{appointment.internal_notes}"
