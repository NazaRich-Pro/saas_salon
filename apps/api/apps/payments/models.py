...
class Payment(models.Model):
    ...
    appointment = models.ForeignKey('booking.Appointment', on_delete=models.SET_NULL, null=True, blank=True, related_name='payments')
    ...
    # Amount
    amount_kgs = models.DecimalField(max_digits=10, decimal_places=2, validators=[MinValueValidator(0)])
    
    # External reference (from payment gateway)
    external_ref = models.CharField(max_length=255, blank=True, help_text='External payment ID from gateway')
    
    # Metadata
    metadata = models.JSONField(default=dict, blank=True, help_text='Additional payment metadata')
    
    # Audit
    processed_by = models.ForeignKey('users.User', on_delete=models.SET_NULL, null=True, blank=True, related_name='processed_payments')
...
class Coupon(models.Model):
    ...
    value = models.DecimalField(max_digits=10, decimal_places=2, help_text='Percentage (0-100) or fixed amount in KGS')
    ...
    #   "time_to": "12:00"
    # }
    rules = models.JSONField(default=dict, blank=True, help_text='Coupon application rules')
...
class GiftCard(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    tenant = models.ForeignKey('tenants.Tenant', on_delete=models.CASCADE, related_name='gift_cards')
    
    code = models.CharField(max_length=50, unique=True, db_index=True)
    balance_kgs = models.DecimalField(max_digits=10, decimal_places=2, validators=[MinValueValidator(0)])
    initial_balance_kgs = models.DecimalField(max_digits=10, decimal_places=2)
    
    # Owner (optional)
    owner_customer = models.ForeignKey('booking.Customer', on_delete=models.SET_NULL, null=True, blank=True, related_name='gift_cards')
...
class LoyaltyRule(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    tenant = models.OneToOneField('tenants.Tenant', on_delete=models.CASCADE, related_name='loyalty_rule')
    
    # Earning rules
    earn_per_100_kgs = models.IntegerField(default=1, help_text='Points earned per 100 KGS spent')
    ...
    min_points_to_redeem = models.IntegerField(default=100, help_text='Minimum points to redeem')
...
class SaaSSubscription(models.Model):
    ...
    # Pricing
    monthly_price_kgs = models.DecimalField(max_digits=10, decimal_places=2, default=500)
