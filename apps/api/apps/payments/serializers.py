...
from .models import Payment, Coupon, GiftCard, LoyaltyRule, SaaSSubscription
...
class PaymentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Payment
        fields = [
            'id', 'appointment', 'appointment_info', 'type', 'provider', 'status',
            'amount_kgs', 'external_ref', 'metadata',
...
class MarkCashPaidSerializer(serializers.Serializer):
    ...
    amount_kgs = serializers.DecimalField(max_digits=10, decimal_places=2, min_value=Decimal('0.01'))
...
class CouponSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Coupon
        fields = [
            'id', 'code', 'kind', 'value', 'valid_from', 'valid_to',
            'max_uses', 'uses_count', 'uses_remaining', 'max_uses_per_customer',
            'rules', 'is_active', 'is_valid', 'created_at'
        ]
...
class GiftCardSerializer(serializers.ModelSerializer):
    ...
    owner_name = serializers.CharField(source='owner_customer.name', read_only=True, allow_null=True)
    
    class Meta:
        model = GiftCard
        fields = [
            'id', 'code', 'balance_kgs', 'initial_balance_kgs',
            'owner_customer', 'owner_name', 'valid_from', 'valid_to',
...
class LoyaltyRuleSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = LoyaltyRule
        fields = [
            'id', 'earn_per_100_kgs', 'redeem_rate', 'min_points_to_redeem',
...
class SaaSSubscriptionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'tenant', 'tenant_name', 'plan', 'seats', 'status',
            'period_start', 'period_end', 'grace_until', 'monthly_price_kgs',
            'last_payment_date', 'next_payment_date', 'days_until_expiry',
            'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'created_at', 'updated_at']
