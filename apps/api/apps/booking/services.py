...
from .models import Appointment, AppointmentService, Service, Staff, Customer, Location
...
class AppointmentService:
    ...
    def reschedule_appointment(
        ...
        if new_staff_id:
            try:
                staff = Staff.objects.get(id=new_staff_id, tenant=self.tenant, is_active=True)
            except Staff.DoesNotExist:
                raise AppointmentCreationError('Staff member not found')
        else:
        ...
        appointment.save(update_fields=['staff', 'start_at', 'end_at', 'status', 'updated_at'])
    ...
    def complete_appointment(self, appointment_id: str) -> Appointment:
        ...
        with transaction.atomic():
            ...
            try:
                loyalty_rule = LoyaltyRule.objects.get(tenant=self.tenant, is_active=True)
                points_earned = int(appointment.total_price_kgs / 100) * loyalty_rule.earn_per_100_kgs
                customer.loyalty_points += points_earned
            except LoyaltyRule.DoesNotExist:
                pass
            
            customer.save(update_fields=['total_visits', 'total_spent_kgs', 'loyalty_points'])
