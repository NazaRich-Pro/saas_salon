...
from .models import Schedule, Appointment, Service, Staff, StaffService
...
class SlotGenerator:
    ...
    def _get_staff_slots(
        ...
        exception = next(
            (exc for exc in schedule.exceptions if exc.get('date') == date_str),
            None
        )
