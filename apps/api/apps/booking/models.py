...
class ServiceCategory(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    tenant = models.ForeignKey('tenants.Tenant', on_delete=models.CASCADE, related_name='service_categories')
...
class Service(models.Model):
    ...
    price_kgs = models.DecimalField(max_digits=10, decimal_places=2, help_text='Price in KGS')
    
    # Buffer times
    buffer_before_min = models.IntegerField(default=0, help_text='Buffer before appointment in minutes')
    buffer_after_min = models.IntegerField(default=0, help_text='Buffer after appointment in minutes')
    
    # Settings
    allow_combo = models.BooleanField(default=True, help_text='Allow combining with other services')
    is_active = models.BooleanField(default=True)
...
class Staff(models.Model):
    ...
    user = models.ForeignKey('users.User', on_delete=models.SET_NULL, null=True, blank=True, related_name='staff_profiles')
    ...
    # Professional info
    title = models.CharField(max_length=100, blank=True, help_text='e.g., Senior Stylist')
...
class StaffService(models.Model):
    ...
    # Overrides (optional)
    duration_override_min = models.IntegerField(null=True, blank=True, help_text='Override service duration')
    price_override_kgs = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, help_text='Override service price')
...
class Schedule(models.Model):
    ...
    # ]
    exceptions = models.JSONField(default=list, blank=True, help_text='Date-specific exceptions')
...
class Customer(models.Model):
    ...
    # Additional info
    date_of_birth = models.DateField(null=True, blank=True, help_text='For birthday campaigns')
...
class Appointment(models.Model):
    ...
    location = models.ForeignKey(Location, on_delete=models.SET_NULL, null=True, blank=True, related_name='appointments')
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['staff', 'start_at', 'end_at']),  # For double-booking prevention
        ]
...
class AppointmentService(models.Model):
    ...
    price_kgs = models.DecimalField(max_digits=10, decimal_places=2, help_text='Actual price for this appointment')
