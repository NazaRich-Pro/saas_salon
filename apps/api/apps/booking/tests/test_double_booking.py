...
from apps.booking.models import (
    Service, ServiceCategory, Staff, StaffService, Schedule,
    Customer, Appointment
)
from apps.booking.services import AppointmentService, AppointmentCreationError
...
def staff_service(db, staff, service):
    return StaffService.objects.create(
        staff=staff,
        service=service
    )
...
class TestDoubleBookingPrevention:
    ...
    def test_prevent_overlapping_appointments(self, tenant, staff, service, staff_service, schedule):
    ...
    def test_allow_non_overlapping_appointments(self, tenant, staff, service, staff_service, schedule):
        """Test that non-overlapping appointments are allowed"""
        appointment_service = AppointmentService(tenant)
        
        # Create first appointment at 10:00
        start_time1 = timezone.now().replace(hour=10, minute=0, second=0, microsecond=0) + timedelta(days=1)
    ...
    @pytest.mark.slow
    def test_concurrent_booking_prevention(self, tenant, staff, service, staff_service, schedule):
        ...
        assert len(successful_appointments) == 1, f"Expected 1 successful booking, got {len(successful_appointments)}"
...
class TestSlotGeneration:
    ...
    def test_generate_slots_for_working_day(self, tenant, staff, service, staff_service, schedule):
    ...
    def test_no_slots_for_non_working_day(self, tenant, staff, service, staff_service, schedule):
    ...
    def test_slots_exclude_existing_appointments(self, tenant, staff, service, staff_service, schedule):
...
class TestAppointmentLifecycle:
    ...
    def test_create_pending_appointment(self, tenant, staff, service, staff_service, schedule):
    ...
    def test_complete_appointment_updates_customer_stats(self, tenant, staff, service, staff_service, schedule):
    ...
    def test_reschedule_appointment(self, tenant, staff, service, staff_service, schedule):
...
class TestScheduleRules:
    ...
    def test_respect_working_hours(self, tenant, staff, service, staff_service, schedule):
    ...
    def test_schedule_exception_day_off(self, tenant, staff, service, staff_service, schedule):
    ...
    def test_schedule_exception_custom_hours(self, tenant, staff, service, staff_service, schedule):
