...
def revenue_report(request):
    ...
    if group_by == 'day':
        ...
        services_data = AppointmentService.objects.filter(
...
def no_show_report(request):
    ...
    # No-shows by staff
    no_shows_by_staff = appointments.filter(
        status='NO_SHOW'
    ).values(
        'staff__id',
        'staff__name'
    ).annotate(
        no_show_count=Count('id')
    ).order_by('-no_show_count')
    
    # No-shows by day
    no_shows_by_day = appointments.filter(
        status='NO_SHOW'
    ).annotate(
        day=TruncDate('start_at')
    ).values('day').annotate(
        count=Count('id')
    ).order_by('day')
...
def kpi_report(request):
    ...
    cash_revenue = payments.filter(type='CASH').aggregate(Sum('amount_kgs'))['amount_kgs__sum'] or Decimal('0.00')
    card_revenue = payments.filter(type='CARD').aggregate(Sum('amount_kgs'))['amount_kgs__sum'] or Decimal('0.00')
    online_revenue = payments.filter(type='ONLINE').aggregate(Sum('amount_kgs'))['amount_kgs__sum'] or Decimal('0.00')
    
    # Customer metrics
    unique_customers = appointments.values('customer').distinct().count()
    repeat_customers = appointments.values('customer').annotate(
...
def export_csv(request):
    ...
    appointments = Appointment.objects.filter(
        ...
        start_at__date__gte=from_date,
        start_at__date__lte=to_date
    ).select_related('customer', 'staff').order_by('start_at')
    ...
    response['Content-Disposition'] = f'attachment; filename="appointments_{from_date}_{to_date}.csv"'
...
def payments_export_csv(request):
    ...
    # Create CSV response
    response = HttpResponse(content_type='text/csv; charset=utf-8-sig')
    response['Content-Disposition'] = f'attachment; filename="payments_{from_date}_{to_date}.csv"'
    ...
    for payment in payments:
        customer_name = payment.appointment.customer.name if payment.appointment else 'N/A'
