...
def archive_old_appointments(self):
    ...
    try:
        ...
        logger.info(f"Archived {archived_count} old appointments (older than {cutoff_date})")
    ...
    try:
        ...
        for tenant in tenants:
            try:
                ...
                # Top staff
                top_staff = appointments.filter(status='COMPLETED').values(
                    'staff__name'
                ).annotate(
                ...
                logger.info(f"Sent daily digest to {admins.count()} admins for tenant {tenant.slug}")
            
            except Exception as e:
                logger.error(f"Error sending digest for tenant {tenant.slug}: {e}")
                continue
        
        logger.info(f"Daily digest task completed. Sent {digests_sent} emails to admins.")
