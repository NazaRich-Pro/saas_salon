...
from apps.tenants.models import Tenant, TenantDomain, Membership
from apps.booking.models import (
    Location, ServiceCategory, Service, Staff, StaffService,
    Schedule, Customer, Appointment, AppointmentService
)
from apps.payments.models import LoyaltyRule, SaaSSubscription, Coupon
...
class Command(BaseCommand):
    ...
    def _flush_demo_data(self):
        ...
        User.objects.filter(email__in=['admin@saas.akylman.online', 'salon@demo.com', 'solo@demo.com']).delete()
    ...
    def _create_demo_salon(self):
        ...
        staff_list = [
            ...
            ('Мария Сидорова', 'Мастер маникюра', '+996700333333', 'maria@demo.com', 20),
        ]
        ...
        customers = [
            ...
            ('Гульмира Жумабаева', '+996555333333', 'gulmira@example.com', date(1995, 11, 5)),
            ('Дастан Эрматов', '+996555444444', 'dastan@example.com', date(1992, 1, 10)),
        ]
        ...
        templates = [
            ('WELCOME', 'EMAIL', 'RU', 'Добро пожаловать в %salon_name%!', 
             'Здравствуйте, %owner_name%!\nВаш салон успешно создан: %tenant_url%\nЛогин: %email%'),
            ('REMINDER_24H', 'EMAIL', 'RU', 'Напоминание о записи', 
             'Здравствуйте, %customer_name%!\nНапоминаем о вашей записи завтра в %time% к мастеру %staff_name%.'),
            ('REMINDER_2H', 'EMAIL', 'RU', 'Напоминание о записи через 2 часа', 
    ...
    def _create_demo_solo(self):
        ...
        Schedule.objects.get_or_create(
            ...
            defaults={
                'rules': {
                    ...
                    'saturday': {'enabled': True, 'slots': [{'start': '11:00', 'end': '18:00'}]},
                    'sunday': {'enabled': False, 'slots': []}
                },
                'exceptions': [],
                'is_active': True
        ...
        self.stdout.write(self.style.SUCCESS(f'✅ Demo solo master "{solo.name}" created successfully!'))
        return solo
