...
class TenantMiddleware:
    ...
    def __call__(self, request):
        ...
        if tenant_id:
            try:
                request.tenant = Tenant.objects.get(id=tenant_id, status__in=['TRIAL', 'ACTIVE', 'GRACE'])
        ...
        if request.tenant:
            logger.debug(f"Resolved tenant: {request.tenant.name} (slug: {request.tenant.slug}) from host: {host}")
        else:
    ...
    def _resolve_tenant(self, host, primary_domain):
        ...
        try:
            ...
            # Case 3: Custom domain (white-label)
                ...
                tenant_domain = TenantDomain.objects.select_related('tenant').filter(
                    domain=host,
                ...
                if tenant_domain:
                    logger.info(f"Resolved custom domain: {host} → {tenant_domain.tenant.name}")
